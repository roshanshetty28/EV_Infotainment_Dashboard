...
{ 
  ...
  regBank.add(40001);     //input for a/c slider(note: set 6x and 1 in EBPro software for slider to get data)
  pinMode(RedPin,OUTPUT);//Red Light
  pinMode(YellowPin,OUTPUT);//Yellow Light
  pinMode(GreenPin,OUTPUT);//Green light
  pinMode(ENA, OUTPUT);//enable pin of L293D
  pinMode(IN1, OUTPUT);//pin 1 of motor
  pinMode(IN2, OUTPUT);// pin 2 of motor
  digitalWrite(IN1, LOW);//turning the motor ON with speed 0(continued below)
  digitalWrite(IN2, HIGH);//the motor starts rotating after enable is increased
...
unsigned int i = regBank.get(40001);    //for A/C motor control
analogWrite(ENA,i);
int rpm=21.96*i;
